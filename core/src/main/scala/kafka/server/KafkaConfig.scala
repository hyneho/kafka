...
class KafkaConfig private(doLog: Boolean, val props: util.Map[_, _])
  ...
  def remoteLogCopierThreads = getInt(RemoteLogManagerConfig.REMOTE_LOG_MANAGER_COPIER_THREAD_POOL_SIZE_PROP)
  def remoteLogExpirationThreads = getInt(RemoteLogManagerConfig.REMOTE_LOG_MANAGER_EXPIRATION_THREAD_POOL_SIZE_PROP)
  def remoteLogReaderThreads = getInt(RemoteLogManagerConfig.REMOTE_LOG_READER_THREADS_PROP)
