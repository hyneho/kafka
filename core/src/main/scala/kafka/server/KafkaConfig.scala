...
class KafkaConfig private(doLog: Boolean, val props: util.Map[_, _])
  ...
  def gzipCompressionBuffer = getInt(ServerConfigs.COMPRESSION_GZIP_BUFFER_CONFIG)
  def snappyCompressionBlock = getInt(ServerConfigs.COMPRESSION_SNAPPY_BLOCK_CONFIG)
  def lz4CompressionLevel = getInt(ServerConfigs.COMPRESSION_LZ4_LEVEL_CONFIG)
  def lz4CompressionBlock = getInt(ServerConfigs.COMPRESSION_LZ4_BLOCK_CONFIG)
  def zstdCompressionLevel = getInt(ServerConfigs.COMPRESSION_ZSTD_LEVEL_CONFIG)
  def zstdCompressionWindow = getInt(ServerConfigs.COMPRESSION_ZSTD_WINDOW_CONFIG)
  ...
  def extractLogConfigMap: java.util.Map[String, Object] = {
    ...
    logProps.put(TopicConfig.COMPRESSION_ZSTD_WINDOW_CONFIG, zstdCompressionWindow)
