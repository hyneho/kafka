...
class TransactionsExpirationTest extends KafkaServerTestHarness {
  ...
  def testBumpTransactionalEpochAfterInvalidProducerIdMapping(quorum: String, groupProtocol: String, isTV2Enabled: Boolean): Unit = {
    producer.initTransactions()

    System.out.println("-----test 1")
    // Start and then abort a transaction to allow the transactional ID to expire.
    producer.beginTransaction()
    // To make sure the producer will be able to pick
    producer.send(TestUtils.producerRecordWithExpectedTransactionStatus(topic1, 0, "2", "2", willBeCommitted = false)).get()
    producer.send(TestUtils.producerRecordWithExpectedTransactionStatus(topic2, 0, "4", "4", willBeCommitted = false)).get()
