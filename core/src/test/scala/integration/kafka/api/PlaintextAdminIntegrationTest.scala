    ...
    try {
      ...
      TestUtils.waitUntilTrue(() => {
        ...
        describeConsumerGroupResult.containsKey(testGroupId) &&
          describeConsumerGroupResult.get(testGroupId).groupState == GroupState.RECONCILING &&
          describeConsumerGroupResult.get(testGroupId).members.size == 2
      }, s"Expected to find 2 members in group $testGroupId")
      ...
      assertEquals(Optional.of(1), classicMember.get.memberEpoch)
