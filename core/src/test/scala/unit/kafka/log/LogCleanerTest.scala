...
import org.apache.kafka.storage.internals.log.{AbortedTxn, AppendOrigin, CleanerConfig, LocalLog, LogAppendInfo, LogConfig, LogDirFailureChannel, LogFileUtils, LogLoader, LogSegment, LogSegments, LogStartOffsetIncrementReason, OffsetMap, ProducerStateManager, ProducerStateManagerConfig, UnifiedLog => JUnifiedLog}
...
class LogCleanerTest extends Logging {
  ...
  def testCleanSegmentsWithConcurrentSegmentDeletion(): Unit = {
    ...
    val leaderEpochCache = JUnifiedLog.maybeCreateLeaderEpochCache(
      dir, topicPartition, logDirFailureChannel, config.recordVersion, "", Optional.empty(), time.scheduler)
