...
class KafkaConfigTest {
  ...
  def testInvalidGzipCompressionLevel(): Unit = {
    ...
    props.setProperty(ServerConfigs.COMPRESSION_GZIP_LEVEL_CONFIG, (CompressionType.GZIP_MAX_LEVEL + 1).toString)
    assertThrows(classOf[ConfigException], () => KafkaConfig.fromProps(props))
  }
  ...
  def testInvalidLz4CompressionLevel(): Unit = {
    ...
    props.setProperty(ServerConfigs.COMPRESSION_LZ4_LEVEL_CONFIG, (CompressionType.LZ4_MAX_LEVEL + 1).toString)
    assertThrows(classOf[ConfigException], () => KafkaConfig.fromProps(props))
  }
  ...
  def testInvalidZstdCompressionLevel(): Unit = {
    ...
    props.setProperty(ServerConfigs.COMPRESSION_ZSTD_LEVEL_CONFIG, (CompressionType.ZSTD_MAX_LEVEL + 1).toString)
    assertThrows(classOf[ConfigException], () => KafkaConfig.fromProps(props))
  }
  ...
  def testFromPropsInvalid(): Unit = {
    ...
    KafkaConfig.configNames.foreach { name =>
      name match {
        ...
        case ServerConfigs.COMPRESSION_ZSTD_LEVEL_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_number", CompressionType.ZSTD_MAX_LEVEL + 1)
  ...
  def testDynamicLogConfigs(): Unit = {
    ...
    ServerTopicConfigSynonyms.TOPIC_CONFIG_SYNONYMS.forEach { case (logConfig, kafkaConfigProp) =>
      logConfig match {
        ...
        case TopicConfig.COMPRESSION_GZIP_BUFFER_CONFIG =>
          assertDynamic(kafkaConfigProp, 512 + 1, () => config.gzipCompressionBuffer)
        case TopicConfig.COMPRESSION_SNAPPY_BLOCK_CONFIG =>
          assertDynamic(kafkaConfigProp, 32768 + 1, () => config.snappyCompressionBlock)
        case TopicConfig.COMPRESSION_LZ4_LEVEL_CONFIG =>
          assertDynamic(kafkaConfigProp, "5", () => config.lz4CompressionLevel)
        case TopicConfig.COMPRESSION_LZ4_BLOCK_CONFIG =>
          assertDynamic(kafkaConfigProp, 4 + 1, () => config.lz4CompressionBlock)
        case TopicConfig.COMPRESSION_ZSTD_LEVEL_CONFIG =>
          assertDynamic(kafkaConfigProp, "5", () => config.zstdCompressionLevel)
        case TopicConfig.COMPRESSION_ZSTD_WINDOW_CONFIG =>
          assertDynamic(kafkaConfigProp, 10, () => config.zstdCompressionWindow)
