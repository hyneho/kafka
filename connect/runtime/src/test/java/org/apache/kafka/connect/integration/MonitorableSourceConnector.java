...
public class MonitorableSourceConnector extends TestableSourceConnector {
    ...
    public void start(Map<String, String> props) {
        super.start(props);
        log.info("starting MonitorableSourceConnector");
        PluginMetrics pluginMetrics = context.pluginMetrics();
        metricsName = pluginMetrics.metricName("start", "description", Collections.emptyMap());
    ...
    public static class MonitorableSourceTask extends TestableSourceTask {
        ...
        public void start(Map<String, String> props) {
            super.start(props);
            log.info("starting MonitorableSourceTask");
            PluginMetrics pluginMetrics = context.pluginMetrics();
            log.info("task pluginMetric {}", pluginMetrics.hashCode());
            metricsName = pluginMetrics.metricName("poll", "description", Collections.emptyMap());
        ...
        public List<SourceRecord> poll() {
            ...
            if (records != null) {
                log.info("poll with {} records", records.size());
                for (SourceRecord record : records) {
                    count++;
                }
            } else {
                log.info("null records");
            }
            return records;
        }
