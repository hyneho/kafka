...
IF EXIST "%BASE_DIR%/config/connect-log4j.properties" (
    ...
    echo You can also use the %BASE_DIR%/config/connect-log4j2.yml file as a starting point. Make sure to remove the Log4j 1.x configuration after completing the migration.
    set KAFKA_LOG4J_OPTS=-Dlog4j.configuration=file:%BASE_DIR%/config/connect-log4j.properties
) ELSE IF EXIST "%BASE_DIR%/config/connect-log4j2.yml" (
    set KAFKA_LOG4J_OPTS=-Dlog4j2.configurationFile=%BASE_DIR%/config/connect-log4j2.yml
